...
class ModelRunner:
  ...
  def run(self, input_images, input_texts, output_text_len=128, generate_image=False,
    ...
    if not answer_options:
      ...
      if self.log_inputs:
        if isinstance(answer_options[0], np.ndarray):
          logging.info(f"Running model text_inputs={input_texts} and "
                       f"{output_options.shape[1]} image answer options")
        else:
          logging.info(f"Running model text_inputs={input_texts} and "
                f"{output_options.shape[1]} answer options: {answer_options}")
  ...
  def caption(self, image, num_decodes=None) -> Dict:
    ...
    if type(image) == list:
      out = self.run(image, [CAPTIONING_PROMPT]*len(image), output_text_len=32,
                     generate_image=False, num_decodes=num_decodes)
      return {k: out[k] for k in ["text", "score"]}
    else:
  ...
  def vqa(self, image, question, num_decodes=None) -> Dict:
    ...
    if type(question) == list:
      out = self.run(image, [q.lower() for q in question], output_text_len=32,
                     generate_image=False, num_decodes=num_decodes)
      return {k: out[k] for k in ["text", "score"]}
    else:
  ...
  def refexp(self, image, expression, num_decodes=None) -> Dict:
    ...
    if type(expression) == list:
      prompts = [REFEXP_PROMPT.replace("{}", expr) for expr in expression]
      out = self.run(image, prompts, output_text_len=32,
                     generate_image=False, num_decodes=num_decodes)
      print(out)
      return [self._extract_boxes({k: out[k][n] for k in ["text", "score", "text_tokens"]}, image[n].shape) for n in range(0, len(expression))]
    else:
